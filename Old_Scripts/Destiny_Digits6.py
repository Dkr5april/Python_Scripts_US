...
def compute_column_reduction(matrix):
    first_col_sum = sum(matrix[i][0] for i in range(2))  # Sum of first column
    second_col_sum = sum(matrix[i][1] for i in range(2))  # Sum of second column
...
# Compute reductions for combined matrices
c1_col1, c1_col2 = compute_column_reduction(combined_matrix1)
c2_col1, c2_col2 = compute_column_reduction(combined_matrix2)

# Compute final single-digit numbers
final_num1 = reduce_to_single_digit(c1_col1 + c1_col2)
final_num2 = reduce_to_single_digit(c2_col1 + c2_col2)
...
# Print final results
print(f"\nFinal Two Numbers: {final_num1}, {final_num2}")
