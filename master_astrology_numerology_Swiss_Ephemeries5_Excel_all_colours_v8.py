...
5. FIX: Robust date handling using dayfirst=True to resolve UserWarnings.
6. NEW: ANSI color codes for aesthetically pleasing console output.
"""
import sys
import random
import pandas as pd
...
def toss_method_5_compound_score(a_name, b_name, a_score, b_score):
    ...
    if b_score > a_score:
        return f"Captain B ({b_name}) - Match Score Advantage ({b_score} > {b_score})" # Changed to B's score
...
def main():
    ...
    try:
        # Load the file (no dayfirst needed here)
    ...
    for index, row in df_input.iterrows():
        try:
            ...
            # Use dayfirst=True in to_datetime here to handle D/M/Y
            match_dt_obj = pd.to_datetime(row.get('Match_Date', '01/01/2025'), dayfirst=True)
            match_date = match_dt_obj.date() 
            match_date_str = match_dt_obj.strftime("%d/%m/%Y") 
            
            a_dob_dt = pd.to_datetime(row.get('Captain_A_DOB', '01/01/1980'), dayfirst=True)
            a_dob = a_dob_dt.strftime("%d/%m/%Y") 

            b_dob_dt = pd.to_datetime(row.get('Captain_B_DOB', '01/01/1980'), dayfirst=True)
    ...
    if all_results:
        ...
        # Reformat date columns to DD/MM/YYYY string explicitly
        ...
        for col in date_cols_to_reformat:
            if col in df_final.columns:
                try:
                    ...
                    print(f"{Colors.WARNING}Warning: Could not reformat column {col} to string. Error: {format_e}{Colors.ENDC}")

        try:
            # Overwrite the file with the new complete data
            df_final.to_excel(IO_FILE, index=False)
            print(f"\n{Colors.OKGREEN}{Colors.BOLD}✅ Analysis complete! All columns (including 6 toss methods and trends) saved to **{IO_FILE}**{Colors.ENDC}")
            ...
            print(f"{Colors.FAIL}Please ensure the Excel file 'match_analysis_data.xlsx' is CLOSED before running the script.{Colors.ENDC}")
    else:
