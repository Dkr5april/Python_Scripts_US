...
def parse_tz_offset(offset_str: str) -> float:
    ...
    # Check for HH:MM format (e.g., +10:30, -04:00)
    if ":" in s:
        try:
            sign = -1 if s.startswith('-') else 1
            # Remove sign and split into hours and minutes
            parts = s.lstrip('+-').split(':')
            hours = float(parts[0])
            minutes = float(parts[1]) if len(parts) > 1 else 0.0
            
            # Convert minutes to fraction of an hour and apply sign
            return sign * (hours + (minutes / 60.0))
        except:
            return 0.0 # Fallback on complex parse failure
    
    # Assume it's already a simple decimal hour (e.g., 4, 11)
    try:
        return float(s)
    except:
        return 0.0 # Fallback on simple parse failure
