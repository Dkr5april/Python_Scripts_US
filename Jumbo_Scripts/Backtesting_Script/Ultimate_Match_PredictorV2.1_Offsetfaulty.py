#!/usr/bin/env python3
"""
Dynamic & Learning-enabled Match Predictor Script
-----------------------------------------------
Reads: match_analysis_data.xlsx (with columns below)
Writes: match_analysis_data_output.xlsx and updates method_stats.json
...
# --- NEW toss method: Planetary Hour Influence (PHI) ---
...
# --- NEW toss method: Elemental Compatibility ---
...
# Print the files being used
    #print(f"{Colors.OKBLUE}Input file: {input_file}{Colors.ENDC}")
    #print(f"{Colors.OKBLUE}Output file: {output_file}{Colors.ENDC}")
    #print(f"{Colors.OKBLUE}Method stats file: {method_stats_file}{Colors.ENDC}")
    print(f"Input file: {input_file}")
    print(f"Output file: {output_file}")
    print(f"Method stats file: {method_stats_file}")
    
    # Load method stats if exists
    if os.path.exists(method_stats_file):
        with open(method_stats_file, "r") as f:
    ...
    try:
        ...
        #print(f"{Colors.FAIL}Failed to write output: {we}{Colors.ENDC}")
        print(f"{Colors.FAIL}Failed to write output: {we}")
        sys.exit(1)
