...
def check_and_revive_methods(method_stats: Dict[str, Any], results_df: pd.DataFrame):
    ...
    if 'Real_Match' in last_window.columns and 'Final_Match_Result' in last_window.columns:
        ...
        if len(predictable_rows) > 0:
            # Need to get the actual result (1 or 2) for comparison
            # We assume Real_Match column contains the name
            # This logic is complex without the full row data, so we use the simpler:
            # Check if overall system confidence is low in the last window
...
def run_predictor_v6():
    ...
    for index, row in df.iterrows():
        ...
        # --- Get raw names for logging clarity ---
        ...
        for method_name, stats in method_stats.items():
            ...
            if is_made:
                ...
                # Check if prediction (1 or 2) matches the actual winner name
                is_correct = (prediction == 1 and team_a_name == actual_winner) or \
                             (prediction == 2 and team_b_name == actual_winner)

                update_method_stats(method_stats, method_name, is_correct, is_made, stats['task'])

        # 7. Enhanced Logging (Shows fallback status)
        log_names = f"[{team_a_name} vs {team_b_name}]"
        
        if not team_a_name_raw and not team_b_name_raw:
             # Use the raw captain names for the clearest fallback logging
             cap_a_display = captain_a_name_raw if captain_a_name_raw != 'N/A' else 'N/A'
             cap_b_display = captain_b_name_raw if captain_b_name_raw != 'N/A' else 'N/A'
             
             log_names = f"[{team_a_name} (Cap A: {cap_a_display}) vs {team_b_name} (Cap B: {cap_b_display})] (NOTE: Team names missing, CAPTAIN names used for prediction!)"
        
        # Log when at least one team name is present, but using the resolved names for prediction.
        elif not team_a_name_raw or not team_b_name_raw:
             log_names = f"[{team_a_name} vs {team_b_name}] (NOTE: One or both team names missing, CAPTAIN names used where needed!)"
