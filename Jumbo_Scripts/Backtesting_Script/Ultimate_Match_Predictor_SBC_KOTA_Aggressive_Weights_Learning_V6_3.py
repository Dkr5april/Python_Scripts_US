...
def run_predictor_v6():
    ...
    try:
        ...
        return
# >>> START: INSERT DEBUG CODE HERE <<<
    print("\n--- DEBUG: Column Names from File ---")
    print(df.columns.tolist())
    print("--------------------------------------\n")
    # >>> END: INSERT DEBUG CODE HERE <<<
    ...
    for index, row in df.iterrows():
        ...
        # V6 FIX: Added an ultimate 'Team A'/'Team B' placeholder to prevent empty log entries.
        team_a_name = safe_str(row_data.get('Team_A_name', row_data.get('Captain_A_Name', 'Team A')))
        team_b_name = safe_str(row_data.get('Team_B_name', row_data.get('Captain_B_Name', 'Team B')))
        ...
        print(f"[{team_a_name} vs {team_b_name}] Toss Winner: {predicted_toss_winner} (Confidence: {toss_confidence:.2f}) -> Match Winner: {predicted_match_winner} (Confidence: {match_confidence:.2f})")
