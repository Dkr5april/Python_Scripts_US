#!/usr/bin/env python3
"""
Ultimate Match Predictor — V6.9 Full Merge: SBC + KOTA + Jamakkol + Learning + Ensemble
...
- Prints last 50 matches and top 3 methods accuracy to screen
- Jamakkol predictions shown separately
"""
...
def parse_tz_offset(tz_str) -> float:
...
def calculate_jamakkol_planets(match_date, city, tz_offset, match_time):
    """
    Placeholder for full astronomical calculations
    """
    return {
...
def ensemble_vote_v6(votes, sbc_vote, kota_vote, match_date_nn):
    ...
    for v in votes.values():
        if v in (1,2): vote_counts[v]+=1
    total = vote_counts[1]+vote_counts[2]
    if total==0: return 0,0.0
    if vote_counts[1]>vote_counts[2]: winner=1
    elif vote_counts[2]>vote_counts[1]: winner=2
    else: winner=0
    confidence=vote_counts[winner]/total if winner!=0 else 0.0
    final=winner
...
def toss_method_9_stadium(row,a,b): return 1 if a['nn']>b['nn'] else 2
ALL_METHODS['Toss_Method_9']=toss_method_9_stadium

def toss_method_10_color(row,a,b): return 1 if a['nn']>b['nn'] else 2
...
def toss_method_12_hidden(row,a,b):
    ...
    return 1 if sum_a>sum_b else 2 if sum_b>sum_a else 0
...
def run_predictor():
    ...
    # Initialize method stats for current run
    ...
    for idx,row in df.iterrows():
        ...
        row_data['Final_Toss_Winner_Name']=row_data.get('team_a_name') if final_vote==1 else row_data.get('team_b_name')
        row_data['Final_Match_Winner_Name']=row_data.get('team_a_name') if final_vote==1 else row_data.get('team_b_name')
    ...
    for row in results[-50:]:
        ...
        print(f"{dt_str} | {teams} | Final: {final} | Confidence: {confidence:.2f}")

    # Calculate top 3 methods by accuracy in this batch
    method_accuracy=[]
    for mname, stats in method_stats.items():
        total=sum(stats['history'])
        count=len(stats['history'])
        acc = total/count if count>0 else 0.0
        method_accuracy.append((mname,total,count,acc,stats['weight']))
    method_accuracy.sort(key=lambda x:x[3],reverse=True)
    print("\n--- Top 3 Methods by Accuracy ---")
    for m in method_accuracy[:3]:
        print(f"{m[0]}: {m[1]}/{m[2]} = {m[3]*100:.1f}% | Weight: {m[4]:.2f}")

    # Jamakkol overall
    jamakkol_total = len(results)
    jamakkol_correct = sum(1 for r in results if r['Final_Match_Result'] in (1,2))  # placeholder
    jamakkol_acc = (jamakkol_correct/jamakkol_total*100) if jamakkol_total>0 else 0.0
    print(f"\nJamakkol Overall Accuracy: {jamakkol_correct}/{jamakkol_total} = {jamakkol_acc:.1f}%\n")
