...
class KPEngineV89:
    ...
    def analyze(self, p, debug=False):
        ...
        hits = [self.p_to_h[p], self.p_to_h[star_lord]] + self.own_map[star_lord]
        hits.append(self.get_h(t_lon))

        # TRANSIT TO NATAL ASPECTS
        asp = []
        for nm, n_lon_birth in self.n_pos.items():
            diff = abs(t_lon - n_lon_birth) % 360
            if diff > 180: diff = 360 - diff
            for ang, name in ALL_ASPECTS:
                if abs(diff - ang) < 2.0:
                    asp.append(f"{name}:{nm}")
        ...
        if debug:
            debug_text = [
                f"Sgn:{sign_lord}", f"Str:{star_lord}", f"Sub:{sub_lord}",
                f"SStar:{substar_lord}", f"Net:{net}", f"Asp:{asp}"
            ]
            return counts, net, sign_lord, star_lord, sub_lord, substar_lord, asp, debug_text
        return counts, net, sign_lord, star_lord, sub_lord, substar_lord, asp
...
def draw():
    table = Table(title=f"KP MASTER ENGINE V89.3 | {engine.view_date.strftime('%Y-%m-%d %H:%M')}")
    table.add_column("Level")
    table.add_column("Planet")
    table.add_column("Lords (Str/Sub/SStr)")
    ...
    for i, p in enumerate(dashas):
        ...
        counts, net, sign, star, sub, substar, asp = res
