...
class KPEngineV89:
    ...
    def analyze(self, p):
        ...
        # ---- KP CHAIN ----
        chain, kp_debug = self.kp_lords(n_lon, depth=4)
        star, sub, subsub, subsubsub = chain

        # ---- SUB LORD STAR ----
        sub_lon = self.n_pos[sub]
        sub_star = PLANET_LIT[int(sub_lon // NAK_LEN) % 9]

        # ---- HOUSE HITS ----
        hits = (
            [self.p_to_h[p], self.p_to_h[star], self.p_to_h[sign_lord]] +
        ...
        return (
            sign, sign_lord, star, sub, subsub, subsubsub, sub_star,
    ...
    def dasha6(self, dt):
        ...
        nak = int(moon // NAK_LEN)
        m_idx = nak % 9

        bal = ((NAK_LEN-(moon%NAK_LEN))/NAK_LEN)*DASHA_YEARS[m_idx]
...
def draw():
    ...
    table.add_column("Sign(L) / Star / Sub / SSub / SSSub [Sub★]")
    if DEBUG_MODE:
    ...
    for i,p in enumerate(dashas):
        s,sl,st,sub,ss,sss,substar,asp,net,dbg = engine.analyze(p)
        row = [
            lvls[i],
            p,
            f"{s}({sl})/{st}/{sub}/{ss}/{sss} [★{substar}]"
        ]
        if DEBUG_MODE:
            row.append(" | ".join(dbg))
