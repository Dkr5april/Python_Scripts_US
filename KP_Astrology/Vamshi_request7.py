...
class AstroEngineV74:
    def __init__(self, dob, tob):
        self.view_date = datetime.now()
        d, m, y = map(int, dob.split("/"))
        hh, mm, ss = map(int, tob.split(":"))
        # STRICTLY USING YOUR INPUT DATE: 05/04/1979
    ...
    # --- NEW CONSOLIDATED LOGIC (Chain up to Sub-Star Lord) ---
    ...
    def get_4_fold_set(self, p_name):
        # Internal helper for consolidation (matches your ranked logic but returns a set)
    ...
    def get_consolidated_strip(self, p_name):
        ...
        # Add Western Aspect House Impacts
    ...
    # --- PRESERVING YOUR ORIGINAL V67 RANKED LOGIC ---
...
engine = AstroEngineV74("05/04/1979", "16:55:00")

def make_layout():
    dasha_list = engine.get_dasha_6(engine.view_date)
    table = Table(expand=True, title=f"KP FULL DEHA CHAIN | DOB: 05/04/1979 | {engine.view_date.strftime('%Y-%m-%d %H:%M')}")
    table.add_column("Level", style="cyan", width=10)
    table.add_column("Planet", style="bold yellow", width=8)
    table.add_column("Ranked Matrix (Old Logic)", width=35)
    table.add_column("Consolidated Strip (New Logic)", width=25)
    ...
    for i, p in enumerate(dasha_list):
        table.add_row(
            lvls[i], p, 
            engine.get_colored_ranked_significators(p), 
            engine.get_consolidated_strip(p), 
