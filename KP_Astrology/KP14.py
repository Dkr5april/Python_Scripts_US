...
# ---------------- DATA ENTRY ----------------
clear_screen()
console.print("[bold cyan]KP ENGINE V55 - TABBED VIEW[/bold cyan]")
u_name = console.input("Name: ")
u_dob  = console.input("DOB (DD/MM/YYYY): ")
u_tob  = console.input("TOB (HH:MM:SS): ")
u_city = console.input("City: ")
...
class AstroEngineV55:
    def __init__(self, dob, tob, lat, lon):
        ...
        self.current_tab = "CHART"  # Tabs: CHART, DATA
...
def render_chart_tab():
    ...
    def cell(n):
        txt = f"[green]{' '.join(grid[n]['n'])}[/]\n[red]{' '.join(grid[n]['t'])}[/]"
        return Panel(txt, title=f"H{n}", height=6)
...
def render_data_tab():
    table = Table(title="Full Transit & Natal Degrees", expand=True)
    table.add_column("Planet", style="cyan")
    table.add_column("Natal Pos", style="green")
    table.add_column("Transit Pos", style="red")
    table.add_column("House (KP)", style="yellow")
    
    trans = engine.get_transit_data(engine.view_date)
    for p in PLANET_LIT:
        n_lon = engine.n_pos[p]["lon"]
        t_lon = trans[p]["lon"]
        table.add_row(p, f"{n_lon:.2f}°", f"{t_lon:.2f}°", "Coming Soon")
    return table

def make_layout():
    layout = Layout()
    header = f"VIEW: {engine.current_tab} | [TAB] Switch | [Arrows] Time | [Q] Quit"
    layout.split_column(
        Layout(Panel(header, style="white on blue"), size=3),
        Layout(name="main")
    )
    
    if engine.current_tab == "CHART":
        layout["main"].update(render_chart_tab())
    else:
        layout["main"].update(render_data_tab())
    return layout
...
def on_press(key):
    ...
    try:
        if key == keyboard.Key.tab:
            engine.current_tab = "DATA" if engine.current_tab == "CHART" else "CHART"
