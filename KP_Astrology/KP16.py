...
class AstroEngineV55:
    ...
    def get_4fold_significators(self, is_transit=False):
        ...
        # Level B: Occupants & Level A: Star Lord of Occupant
        for p, data in positions.items():
            h_occ = int(data['lon'] / 30) + 1
            sig_data[h_occ]["B"].append(p)
            
            s_lord = self.get_star_lord(data['lon'])
            sig_data[h_occ]["A"].append(s_lord)

        # Level D: House Lord & Level C: Star Lord of House Lord
        for i in range(12):
            house_nr = i + 1
            h_lord = RASHI_LORDS[int(self.n_cusps[i] / 30)]
            sig_data[house_nr]["D"].append(h_lord)
            
            # Star Lord of the House Lord (Level C)
            lord_lon = positions[h_lord]["lon"]
            sig_data[house_nr]["C"].append(self.get_star_lord(lord_lon))
