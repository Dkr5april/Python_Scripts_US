...
# Major Western Aspects and their Orbs
ASPECTS = [
    (0, "Conj"),   # Conjunction
    (60, "Sext"),  # Sextile
    (90, "Squa"),  # Square
...
class AstroEngineV65:
    ...
    def get_aspects(self, p_name):
        ...
        for n_p, n_lon in self.n_pos.items():
            ...
            for angle, name, orb in [(a[0], a[1], 8) for a in ASPECTS]:
                if abs(diff - angle) <= orb:
                    found.append(f"{name} {n_p}")
...
def make_layout():
    ...
    table = Table(expand=True, title=f"KP RANKED MATRIX | {engine.view_date.strftime('%Y-%m-%d %H:%M')}")
    ...
    for i, p in enumerate(dasha_list):
        table.add_row(lvls[i], p, engine.get_ranked_significators(p), engine.get_aspects(p))
