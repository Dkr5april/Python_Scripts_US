...
class KPEngineV89:
    def __init__(self, dob, tob, lat, lon):
        ...
        # Calculate birth_moon immediately to prevent AttributeError
...
def on_p(key):
    global exit_f
    try:
        if key == keyboard.Key.right: engine.view_date += timedelta(days=1); engine.refresh()
        if key == keyboard.Key.left: engine.view_date -= timedelta(days=1); engine.refresh()
        if hasattr(key, 'char') and key.char == 'q': exit_f = True
    except: pass

def draw():
    table = Table(title=f"KP MASTER ENGINE V89.9 | {engine.view_date.strftime('%Y-%m-%d %H:%M')}", expand=True)
