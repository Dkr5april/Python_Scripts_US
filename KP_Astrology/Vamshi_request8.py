...
class AstroEngineV75:
    ...
    # --- UPDATED DASHA LOGIC (Synced with RVA PDF) ---
    def get_dasha_6(self, t_dt):
        ...
        # First Lord is determined by Moon Star (Jupiter for Punarvasu)
        lord_id = star_idx % 9 
        
        # Calculate birth balance to match RVA report (Jupiter ends 28-04-1979)
        rem_deg = one_star - (moon_lon % one_star)
        # 365.2425 ensures we don't drift over decades
        days_in_cycle = 120 * 365.2425
        
        # Start of current 120-year cycle
        ...
        cycle_start = m_start
        while t_jd > (cycle_start + days_in_cycle): cycle_start += days_in_cycle
        return recurse(cycle_start, days_in_cycle, lord_id, 0)

    # --- CONSOLIDATED STRIP (Includes Sub-Star and Western Impacts) ---
    ...
    def get_consolidated_strip(self, p_name):
        ...
        # Injection of Western Aspect impacts
        t_lon = self.t_pos[p_name]
        for n_p, n_lon in self.n_pos.items():
            diff = abs(t_lon - n_lon) % 360
            if diff > 180: diff = 360 - diff
            for angle, name, col in ALL_ASPECTS:
                if abs(diff - angle) <= 5: combined.add(self.p_to_h[n_p])

        strip = Text()
        for i, h in enumerate(sorted(list(combined))):
            strip.append(str(h), style=HOUSE_COLORS.get(h, "white"))
            if i < len(combined)-1: strip.append(",", style="white")
        return strip
...
engine = AstroEngineV75("05/04/1979", "16:55:00")

def make_layout():
    d_list = engine.get_dasha_6(engine.view_date)
    table = Table(expand=True, title=f"KP SYNCED ENGINE V75 | {engine.view_date.strftime('%Y-%m-%d %H:%M')}")
    table.add_column("Level", style="cyan", width=10)
    table.add_column("Planet", style="bold yellow", width=8)
    table.add_column("Ranked Matrix (Old)", width=35)
    table.add_column("Consolidated (New)", width=30)
    ...
    return Layout(Panel(table, subtitle="[Arrows] Time | [Q] Quit | SYNCED TO RVA PDF"))
