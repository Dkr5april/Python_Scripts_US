# EON_Differential_Pair_Analysis.py
...
# ... (other vibe functions are similar, skipping for brevity in this display)
...
def get_all_risk_scores(date, chart, birth_root, destiny_nums):
...
def generate_differential_report(person_data_list, target_dates):
    ...
    for date in target_dates:
        ...
        print(Style.BRIGHT + Fore.CYAN + f"\n════════ TARGET DATE: {date_str} ════════" + Style.RESET_ALL)
        ...
        for i in range(num_pairs):
            ...
            # 1. Calculate Scores and Sums
            scores_A = get_all_risk_scores(date, A['chart'], A['birth_root'], A['destiny_nums'])
            scores_B = get_all_risk_scores(date, B['chart'], B['birth_root'], B['destiny_nums'])
...
# -------------------------
# MAIN CLI LOGIC (NEW: Focused Input)
# -------------------------
