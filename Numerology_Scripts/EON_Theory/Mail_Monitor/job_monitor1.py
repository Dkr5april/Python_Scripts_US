...
# A. Comprehensive Skill Set Categorized
...
# B. Specific Job Titles (Must contain one of these)
TARGET_JOB_TITLES = [
...
# C. Set the minimum filtering thresholds:
...
def analyze_email_for_job(subject: str, body: str) -> dict:
    ...
    # Format the detailed skill categories for the AI prompt
...
# --- 3. IMAP MONITORING LOGIC (Date Filtered) ---

def fetch_unread_emails():
    """Connects to Gmail via IMAP and fetches unread message bodies RECEIVED TODAY."""
    ...
    try:
        ...
        # Search only for emails received TODAY and are UNSEEN
...
def start_monitoring():
    ...
    print("=" * 50)
    print(f"Starting Highly Filtered SAP Job Monitor (Keywords: {MIN_KEYWORDS_REQUIRED}+, Categories: {MIN_CATEGORIES_REQUIRED}+)...")
    print("Checking today's unread inbox every 10 minutes...")
    print("=" * 50)
    ...
    while True:
        ...
        if new_emails:
            print(f"\n[FOUND] {len(new_emails)} new unread email(s) received today. Analyzing...")
            ...
            for i, mail_data in enumerate(new_emails):
                ...
                if ai_result.get("is_relevant_job"):
                    # --- ALERTING LOGIC ---
                    summary = ai_result.get('alert_summary', 'Highly Relevant Job Posting')
                    ...
                    print(f"   Keywords Found: {ai_result.get('matched_keywords_count', 'N/A')}")
            ...
            print(f"[{time.strftime('%H:%M:%S')}] No new unread emails today. Sleeping...")
