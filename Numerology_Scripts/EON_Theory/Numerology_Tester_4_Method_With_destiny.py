...
# --- Global Definitions ---
RISK_MAP = {
    0: (Fore.GREEN, colors.green),    # Good (🟢)
    1: (Fore.YELLOW, colors.yellow),  # Low Risk / Neutral (🟡)
    2: (Fore.BLUE, colors.blue),      # Medium Risk / Significant Work (🔵)
    3: (Fore.RED, colors.red)         # High Risk / Major Conflict (🔴)
}
...
def reduce_to_single_digit(n):
    while n > 9 and n not in [11, 22, 33]: # Allowing Master Numbers for now, but reducing for the daily Vibe
        n = sum(int(d) for d in str(n))
    # We still reduce Master Numbers for the daily vibe comparison (11->2, 22->4, 33->6)
    if n in [11, 22, 33]:
        n = sum(int(d) for d in str(n)) 
    return n
...
def generate_pdf_calendar(name, dob_str, start_year, num_years):
    ...
    for year_offset in range(num_years):
        ...
        # Simplified PDF generation loop (Actual implementation would be much more complex)
        ...
        for month_num in range(1, 13):
            ...
            for day, weekday in cal_iter:
                if day != 0:
                    ...
                    color = eon_risk_pdf(day_date, chart, birth_root)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if not screen_date_str:
                now = datetime.now()
                # Correcting to current month/year based on context time
                screen_month, screen_year = 12, 2025 # Use current context time
    ...
    # --- NEW INPUT: Destiny Alignment ---
    include_destiny = False
    destiny_num = None
    
    while True:
        use_name = input("Do you want to include Name-Based Destiny Alignment (7 Hybrid Systems)? (Y/N): ").upper()
        if use_name == 'Y':
            destiny_num = calculate_destiny_number(name)
            print(Fore.GREEN + f"-> Your Destiny Number is {destiny_num}.")
            include_destiny = True
            break
        elif use_name == 'N':
            print("-> Proceeding with 4 DOB-only numerology comparison.")
            break
        else:
