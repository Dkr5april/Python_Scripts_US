...
def display_month_calendar(chart, birth_root, life_path, month, year):
    ...
    # Calculate starting day and number of days
    num_days = calendar.monthrange(year, month)[1]
    start_weekday = datetime(year, month, 1).weekday() # 0 is Monday
    ...
    for d in range(1, num_days + 1):
        ...
        # Format day number and append with color
        day_str = f"{d:2d}"
        line += f"{fore_color}{day_str}{Style.RESET_ALL} "
        
        # Check if it's Sunday (end of the week)
        if (start_weekday + d) % 7 == 0:
            print(line)
            line = ""
            
    # Print remaining days in the last week
    if line:
        print(line)
...
def generate_pdf_calendar(name, dob_str, start_year, num_years):
    ...
    for year_offset in range(num_years):
        ...
        if year_offset > 0:
            c.showPage() # Start a new page for each subsequent year
        ...
        # Calendar starts below the header/legend elements
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            years_to_generate = 0
            while years_to_generate not in [1, 5, 10]:
                year_choice = input("Generate calendar for 1, 5, or 10 years? (Enter 1, 5, or 10): ")
                if year_choice in ['1', '5', '10']:
                    years_to_generate = int(year_choice)
                else:
                    print(Fore.RED + "Invalid choice. Please enter 1, 5, or 10.")
            break
        except ValueError:
