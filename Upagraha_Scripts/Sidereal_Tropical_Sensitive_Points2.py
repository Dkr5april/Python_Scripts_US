...
geolocator = Nominatim(user_agent="astro_engine_full")
...
class AstroEngine:
    ...
    def calc_planets(self,jd,sid):
        ...
        for pid,n in zip(plist,names):
            ...
            data[n]={"lon":r[0]%360,"dec":r[1],"retro":r[3]<0}
        ...
        for tp,t in t_sid.items():
            for name,lon in {**engine.n_up,**engine.n_apra}.items():
                if ang_diff(t["lon"],lon)<ORB_LON:
                    effect="BAD" if name in ["Dhuma","Vyatipata"] else "GOOD"
                    table.add_row("Shadow",f"{tp}->{name}",effect)

    # --- ANTISCION ---
    if engine.mode=="TROPICAL":
        for tp,t in t_trop.items():
            for np,a in engine.n_ant.items():
                if ang_diff(t["lon"],a)<ORB_LON:
                    table.add_row("Antiscion",f"{tp}-{np}","Hidden support")

    # --- PARALLEL ---
    for tp,t in t_trop.items():
        for np,nv in engine.n_trop.items():
            if abs(t["dec"]-nv["dec"])<ORB_DEC:
                table.add_row("Parallel",f"{tp}-{np}","Strong")
...
def make_layout(engine):
    ...
    layout["body"].split_row(
        ...
        Layout(Panel(impact_table(engine)),ratio=1)
    )
