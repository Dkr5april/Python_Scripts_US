...
def get_impact_table(engine):
    table = Table(title=f"Impacts ({engine.mode})", expand=True)
    ...
    t_planets = engine.calc_planets(tjd, use_sid)
    n_planets = engine.n_sid if use_sid else engine.n_trop

    found = False

    for tp, t in t_planets.items():
        for name, lon in {**engine.n_up, **engine.n_apra}.items():
            if ang_diff(t["lon"], lon) < ORB_LON:
                ...
                if name in ["Dhuma","Vyatipata"]:
                    effect = "BAD"
                    meaning = "Obstacles, confusion, karmic pressure"
                elif name in ["IndraChapa","Upaketu"]:
                    effect = "GOOD"
                    meaning = "Insight, sudden help, clarity"
                else:
                    effect = "MIXED"
                    meaning = "Neutral karmic trigger"

                table.add_row(tp, name, effect, meaning)
    ...
    if not use_sid:
        for tp, t in t_planets.items():
            for name, lon in engine.n_ant.items():
                if ang_diff(t["lon"], lon) < ORB_LON:
                    found = True
                    table.add_row(tp, f"Ant.{name}", "GOOD", "Hidden support / opportunity")

    for tp, t in t_planets.items():
        for name, nd in n_planets.items():
            if abs(t["dec"] - nd["dec"]) < ORB_DEC:
                found = True
                table.add_row(tp, f"|| {name}", "STRONG", "Energy amplification")

    if not found:
        table.add_row("-", "-", "NONE", "No major triggers")
...
def get_summary(engine):
    if engine.mode == "SIDEREAL":
        text = "[cyan]Sidereal Mode:[/]\nFocus on karma & destiny\n"
        text += "- Dhuma → obstacles\n- IndraChapa → spiritual gain\n"
    else:
        text = "[green]Tropical Mode:[/]\nFocus on psychology & events\n"
        text += "- Antiscion → hidden support\n- Parallels → strong events\n"

    return Panel(text, title="Interpretation Guide")
...
def make_layout(engine):
    ...
    layout.split_column(
        Layout(Panel(f"{engine.name} | Mode: {engine.mode} | ←/→ | T toggle | Q quit"), size=3),
        Layout(name="body")
    )
    ...
    layout["left"].update(Panel(get_chart(engine), title="Chart"))

    layout["right"].split_column(
        Layout(Panel(get_impact_table(engine))),
        Layout(Panel(get_extra_table(engine))),
        Layout(get_summary(engine))
    )
...
try:
    with Live(make_layout(engine), console=console, refresh_per_second=4, screen=True) as live:
        while not exit_flag:
