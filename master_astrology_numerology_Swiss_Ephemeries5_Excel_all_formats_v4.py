...
def main():
    ...
    for index, row in df_input.iterrows():
        ...
        try:
            ...
            # 'a_name' and 'b_name' now represent the CAPTAIN
            a_name = row.get('Captain_A_Name', team_a)
            # Ensure DOB is read as a string for parsing later
            ...
            try:
                # Try format from Pandas datetime object (YYYY-MM-DD or full datetime string)
                match_date = pd.to_datetime(match_date_raw).date()
                match_date_str_fmt = match_date.strftime("%d/%m/%Y")
            except Exception:
                # Fallback to DD/MM/YYYY user input format
            ...
            try:
                # Combine date and time to create local datetime object
            ...
            if score_a > score_b:
                # Advantage is reported in terms of the Captain's name
            ...
            result = {
                ...
                # Toss Predictions (All methods now specify "Captain A/B")
            ...
            all_results.append({
                ...
                # Fill other columns with error placeholders to prevent missing column issues in DF merge
    ...
    if all_results:
        ...
        # in results for logging/context during error handling.
