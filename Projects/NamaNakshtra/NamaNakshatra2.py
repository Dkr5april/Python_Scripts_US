...
# Standardizes all 'Gunintalu' to base vowels for family matching.
VOWEL_MAP = {
    'ా': 'ఆ', 'ి': 'ఇ', 'ీ': 'ఈ', 'ు': 'ఉ', 'ూ': 'ఊ', 
...
# We group by the root consonant family.
NAKSHATRA_MATRIX = {
    ...
    # Extend this matrix as you test more names
}

def get_nama_nakshatra(input_name):
    # 1. Transliterate to Telugu and normalize 'R'
    tel_name = transliterate.process('autodetect', 'Telugu', input_name).replace("ఱ", "ర")
    
    # 2. Extract first character and vowel sign
    ...
    # 3. Identify Family vs Standalone Vowel
    if '\u0C05' <= char <= '\u0C14': # If it's a primary vowel (అ, ఆ, ఇ...)
        family = "అ"
        vowel_base = char
    else:
        family = char
...
while True:
    name_in = input("\nEnter Name (English/Telugu) or 'exit': ")
    ...
    if info:
        ...
        print("RESULT: Sound family or vowel not yet mapped in NAKSHATRA_MATRIX.")