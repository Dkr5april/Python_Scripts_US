...
class PulippaniEngine:
    ...
    def get_analysis(self):
        ...
        if lagna_rasi in [0,3,6,9]:
            l_type = "Movable"
        elif lagna_rasi in [1,4,7,10]:
            l_type = "Fixed"
        else:
            l_type = "Dual"
        ...
        if l_type in self.grandham.get("lagna_rules", {}):
            ...
            results.append(f"[*] Sloka {sl} (Lagna): {dsc}")
        ...
        for cb in self.grandham.get("combinations", []):
            ...
            for h, present in h_occ.items():
                if target.issubset(present):
                    if cb["house"] == "any" or cb["house"] == h:
                        results.append(
                            f"[+] Sloka {cb['sloka']}: {cb['title']} - {cb['effect']}"
                        )
...
def get_chart_table():
    tjd = swe.julday(
        engine.view_date.year,
        engine.view_date.month,
        engine.view_date.day,
        12.0
    )
...
def make_layout():
    ...
    layout.split_column(
        Layout(Panel(f"Transit: {engine.view_date:%d-%b-%Y} | Zone: {detected_tz}"), size=3),
        Layout(name="body")
    )

    layout["body"].split_row(
        Layout(Panel(get_chart_table(), title="Chart"), ratio=2),
        Layout(Panel("\n".join(engine.get_analysis()), title="Analysis"), ratio=1)
    )
