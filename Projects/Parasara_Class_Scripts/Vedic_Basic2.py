...
class AstroEngine:
    ...
    def get_shadbala_data(self):
        ...
        for p in ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn"]:
            ...
            table_rows.append({
                "Planet": p, "Shadbala": f"{sb:.2f}", "In_Rupas": f"{rupas:.2f}", 
                "Percent_Strength": f"{pct:.2f}%", "IshtaPhala": f"{exact_ishta_kashta[p]['ishta']:.2f}", "KashtaPhala": f"{exact_ishta_kashta[p]['kashta']:.2f}"
            })
        return table_rows
...
def generate_vedic_chart_figure(engine):
    ...
    # Build the 12 Rasis positional array mapped to circular angles
...
def refresh_web_dashboard(n_clicks, selected_planet, dob, tob, city, tdate):
    try:
        # Launch calculations using variables directly parsed from the browser UI input elements
