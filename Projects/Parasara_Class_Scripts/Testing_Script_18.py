...
# --- EXE అప్లికేషన్ పాత్ కరెక్షన్ లాజిక్ ---
if hasattr(sys, '_MEIPASS'):
    # EXE రన్ అవుతున్నప్పుడు టెంపరరీ ఫోల్డర్ పాత్ తీసుకుంటుంది
    ephe_path = os.path.join(sys._MEIPASS, 'ephe')
else:
    # నార్మల్ పైథాన్ స్క్రిప్ట్ రన్ అవుతున్నప్పుడు లోకల్ ఫోల్డర్ తీసుకుంటుంది
...
scroll_offset = 0  # స్క్రోలింగ్ ట్రాక్ చేయడానికి కొత్త వేరియబుల్
...
def render_master_view():
    ...
    if current_screen == "CHART":
        ...
        # విండో సైజుకు సరిపోయేలా గరిష్టంగా 24 లైన్లు మాత్రమే చూపిస్తుంది
        max_visible_lines = 24 
        
        # స్క్రోల్ ఆఫ్ సెట్ బౌండరీలను కంట్రోల్ చేయడం
        ...
        master_panel = Panel(
            scrolled_text,
            title=f"🎯 Parashara Horoscope 10-Steps Matrix (Line {scroll_offset+1} to {scroll_offset+len(visible_lines)} of {len(all_lines)})",
            border_style="yellow",
            padding=(1, 2)
        )
...
def on_press(key):
    global exit_to_report, selected_planet_idx, current_screen, scroll_offset
    try:
        # --- CHART & DETAILS SCREEN: Arrow keys change the active planet ---
        if current_screen in ["CHART", "DETAILS"]:
            if key == keyboard.Key.right and current_screen == "CHART": 
                engine.view_date += timedelta(days=1)
            elif key == keyboard.Key.left and current_screen == "CHART": 
                engine.view_date -= timedelta(days=1)
        ...
        # --- MASTER 10 STEPS SCREEN: Arrow keys scroll the report ---
        elif current_screen == "MASTER_10_STEPS":
            if key == keyboard.Key.down: scroll_offset += 1
            elif key == keyboard.Key.up: scroll_offset -= 1
            
        # --- GLOBAL CHARACTER KEY CONTROLS ---
        if hasattr(key, "char"):
            c = key.char.lower()
