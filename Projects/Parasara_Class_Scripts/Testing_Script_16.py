...
class AstroEngine:
    ...
    # --- NEW ADDITION: SEPARATE 10-STEPS EVALUATION PAGE ---
    def get_master_10_steps_report(self):
        ...
        report_text = Text()
        report_text.append("=== 🎯 MASTER 10-STEPS PARASHARA HOROSCOPE READING ===\n\n")
        
        report_text.append("● Basic Rule (Core Principle):\n")
        report_text.append("  - Jeevamu (Life) = Shubhamu (Kendras/Auspicious houses do good to the living being).\n")
        report_text.append("  - Vriddhi (Growth) = Paapamu (Upachayas give material growth, but bring filtration/inauspiciousness).\n\n")
        
        k_count = sum(len(planets_in_house[h]) for h in [1, 4, 7, 10])
        r1 = "ACTIVE (Jeevamu/Visibility is active. Clear professional/social status)" if k_count >= 2 else "PASSIVE Framework."
        report_text.append(f"● Step 1: Observation of Kendras (Angles) -> Found {k_count} planets in 1st, 4th, 7th, 10th. Status: {r1}\n\n")
        
        h2_ok = bhava_bala_data[2]["rupas"] >= 7.0
        h5_ok = bhava_bala_data[5]["rupas"] >= 7.0
        report_text.append("● Step 2: Panaparas (Succedent houses) and Axis Balance:\n")
        report_text.append(f"  - Principle 1 (Jeevamu): H2 ({bhava_bala_data[2]['rupas']}R) | H5 ({bhava_bala_data[5]['rupas']}R) -> {'Strong: Grants utmost auspiciousness' if (h2_ok and h5_ok) else 'Moderate: Requires self-effort'}\n")
        report_text.append(f"  - Principle 2 & 3 (Vriddhi & Balance): 8-11 Structural axis controls material success parameters with structural balance matrix.\n\n")
        
        u_malefics = sum(1 for h in [3, 6, 10, 11] for p in planets_in_house[h] if p in ["Saturn", "Mars", "Rahu", "Sun"])
        report_text.append(f"● Step 3: Observation of Upachayas -> Found {u_malefics} inauspicious planet(s) in growth houses (3,6,10,11). Conveys Speed & Action in life.\n\n")
        
        lord_11 = self.rasi_lords[(lagna_rasi_idx + 10) % 12]
        h_of_11 = house_of_planet.get(lord_11, 1)
        report_text.append(f"● Step 4: Blemish of the 11th Lord -> 11th Lord ({lord_11}) is sitting in House {h_of_11}. It will filter the \"living significations\" (Jeeva Karakatwas) of this zone while escalating financial growth.\n\n")
        
        report_text.append("● Step 5 & 6: Tattva Trikonas & Life Stages:\n")
        report_text.append("  - Dynamic division into Foundation (H1-H4), Social Relationships (H5-H8), and Universal Purpose (H9-H12) is balanced mathematically.\n\n")
        
        report_text.append("● Step 7: Connecting Bhava Links (Integration):\n")
        report_text.append("  - Evaluation relies on specific House cords, its Lord, Trines (5,9), Sustenance (2nd from it), and Expenditure (12th from it).\n\n")
        
        report_text.append("● Step 8: Determining the Strength of the House (Bhava Bala):\n")
        for h in range(1, 13):
            br = bhava_bala_data[h]["rupas"]
            be = "Wonderful (>8.0 Rupas - Received automatically)" if br >= 8.0 else ("Received with self-effort (7.0-8.0 Rupas)" if br >= 7.0 else "Severe delay and defects (<7.0 Rupas)")
            report_text.append(f"  - House {h}: {br:.2f} Rupas -> {be}\n")
        report_text.append("\n")
        
        report_text.append("● Step 9: Determining Shadbala and Ishta/Kashta Phala (Planetary Power & Quality):\n")
        for p in ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn"]:
            pr = planet_rupas[p]
            ishta = exact_ishta_kashta[p]["ishta"]
            kashta = exact_ishta_kashta[p]["kashta"]
            
            if ishta > 30 and pr >= 5.0: status = "Great Yoga configuration (Favorable results)"
            elif kashta > 30 and pr >= 5.0: status = "Severe difficulties / Heavy structural friction"
            else: status = "Standard Balanced Operational output"
            
            report_text.append(f"  - {p}: Shadbala {pr:.2f} Rupas | Ishta: {ishta/60:.2f} Rupas ({ishta:.2f} Pts) | Kashta: {kashta/60:.2f} Rupas ({kashta:.2f} Pts) -> {status}\n")
        report_text.append("\n")
        
        report_text.append("● Step 10: Dasha - Bhukti Integration & Ultimate Master Principle:\n")
        report_text.append("  - Mahadasha lord rules 70% of lifecycle delivery potential. Bhukti lords govern work pressure and physical execution structures.\n")
        
        sa_aspect = ((lagna_rasi_idx) - tr_sa_rasi) % 12 in [0, 2, 6, 9]
        if sa_aspect:
            report_text.append("  - 🚨 LIVE TRANSIT ASPECT ALERT: Gochar Saturn is casting an aspect on your 1st House/Lagna setup! Demands discipline and administrative shuffling.\n")
        else:
            report_text.append("  - Live Transit: No heavy overlapping malefic transit overrides intersecting the Lagna core right now.\n")
...
def render_master_view():
    ...
    if current_screen == "CHART":
        ...
        header_text.append("🎮 Controls: [C] Back to Kundali Chart  |  [D] View Planet Details  |  [R] Exit to Report", style="magenta bold")
    ...
    if current_screen == "CHART":
        ...
        master_panel = Panel(
            master_report,
            title="🎯 Parashara Horoscope 10-Steps Analytical Matrix",
            border_style="yellow",
            padding=(1, 2)
        )
...
def on_press(key):
    ...
    try:
        ...
        if key == keyboard.Key.down: selected_planet_idx = (selected_planet_idx + 1) % len(PLANET_LIST)
            ...
            elif c == "m": current_screen = "MASTER_10_STEPS"
    except: pass
