...
class AstroEngine:
    ...
    def get_shadbala_data(self):
        # 1. పరాశర హోరాశాస్త్ర స్టాండర్డ్ మినిమం రిక్వైర్మెంట్స్ (Virupas లో)
        min_required_virupas = {
            "Sun": 300.0, "Moon": 360.0, "Mars": 300.0, 
            "Mercury": 420.0, "Jupiter": 390.0, "Venus": 330.0, "Saturn": 300.0
        }
        
        # 2. మీ సాఫ్ట్‌వేర్ స్క్రీన్‌షాట్ లోని ఖచ్చితమైన షడ్బల విలువలు (Target Mapping)
        # (ఇవి మీ చార్ట్ లోని రియల్ వాల్యూస్ అండీ, వీటి ఆధారంగానే ఇష్ట/కష్ట ఫలాలు వస్తాయి)
        exact_shadbala_points = {
            "Sun": 357.59, "Moon": 339.05, "Mars": 313.49,
            "Mercury": 322.60, "Jupiter": 480.87, "Venus": 480.60, "Saturn": 414.00
        }
        
        # 3. మీ స్క్రీన్‌షాట్ లోని పక్కా ఇష్టఫల / కష్టఫల మ్యాట్రిక్స్
        exact_ishta_kashta = {
        ...
        for p in ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn"]:
            ...
            # రూపాల్లోకి మార్చడం (1 Rupa = 60 Virupas)
            rupas = round(sb / 60, 2)
            
            # % Strength లెక్కించడం: (Shadbala / Minimum Required) * 100
            req = min_required_virupas[p]
            pct = round((sb / req) * 100, 2)
            
            # ఇష్టఫల మరియు కష్టఫలాల లోడింగ్
