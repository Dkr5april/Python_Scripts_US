...
console = Console()
geolocator = Nominatim(user_agent="astro_engine_v33")

exit_to_report = False  

# ---------------- INPUT MECHANISM ----------------
console.clear()
console.print("[bold magenta]=== ASTRO ENGINE V33 (100% PURE VEDIC ARTISTIC PREDICTIONS) ===[/bold magenta]\n")
...
# ---------------- COSMIC DICTIONARIES FOR ARTISTIC BLENDING ----------------
GRAHA_KARAKATWAS = {
...
class AstroEngine:
    ...
    def get_shadbala_data(self):
        # 100% పక్కా పరాశర హోరాశాస్త్ర మినిమం రిక్వైర్మెంట్ మ్యాట్రిక్స్
        min_required_virupas = {
            "Sun": 300.0, "Moon": 360.0, "Mars": 300.0, 
            "Mercury": 420.0, "Jupiter": 390.0, "Venus": 330.0, "Saturn": 300.0
        }
        # మీ సాఫ్ట్‌వేర్ లోని రియల్ టైం పాయింట్లు & ఇష్ట/కష్ట ఫలాల మ్యాపింగ్
    ...
    def get_artistic_synthesis(self):
        ...
        for p_name, v in self.n_sid.items():
            ...
            cards.append((p_name, house_num, rasi_name, card_text))
        return cards
...
def show_master_report():
    ...
    # 1. Shadbala, Ishta & Kashta Phala Table Presentation
    ...
    # 2. Pure Vedic Bhava Bala Matrix
...
def make_layout():
    layout = Layout()
    layout.split_column(
        Layout(Panel(f"{name} | {engine.view_date:%d-%b-%Y} | COSMIC CANVAS ARTISTIC BLEND | [yellow]← → Navigate | R Back to Report[/]"),size=3),
        Layout(name="body")
    )
    
    # Combining Artistic Synthesized Interpretation Text
    artistic_text = Text()
    for p_name, h, rasi, txt in engine.get_artistic_synthesis():
        artistic_text.append(f"🌟 {p_name} కూడిన విశ్వ విశ్లేషణ:\n", style="bold underline white")
        artistic_text.append(txt)
        artistic_text.append("\n")

    layout["body"].split_row(
        Layout(Panel(get_chart(),title="Vedic Kundali (Natal: Green | Transit: Red)"),ratio=1), 
        Layout(Panel(artistic_text, title="🎨 Cosmic Weaver: త్రివిధ కారకత్వాల కలయిక"),ratio=1)
    )
    return layout
...
while True:
    show_master_report()
    exit_to_report = False
    input("\nPress ENTER to launch the Cosmic Canvas Real-Time Live Tracker Dashboard...")
