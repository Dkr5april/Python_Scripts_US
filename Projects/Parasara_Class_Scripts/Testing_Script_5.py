...
console = Console()
geolocator = Nominatim(user_agent="astro_engine_v28")
...
FONT_NAME = 'Helvetica'
font_path = "NotoSansTelugu-VariableFont_wdth,wght"  # Adjusted as per your folder filename
...
# ---------------- INPUT ----------------
console.clear()
console.print("[bold magenta]=== ASTRO ENGINE V28 (PERFECT RAHU-KETU & DYNAMIC STEP VALIDATIONS) ===[/bold magenta]\n")
...
class AstroEngine:
    ...
    def calc_planets(self, jd, sidereal):
        ...
        data = {}
        # Calculate standard 7 planets + Rahu directly from Ephe
        for p_name, pid in PLANET_IDS.items():
            res = swe.calc_ut(jd, pid, flags)[0]
            lon, vel = res[0], res[3]
            data[p_name] = {"lon": lon, "retro": vel < 0}
        
        # Ketu is always exactly 180 degrees ahead of Rahu
    ...
    def get_10_step_data(self):
        ...
        s1 = f"Kendra Houses (1,4,7,10) contain {len(k_planets)} planets: ({', '.join(k_planets) if k_planets else 'None'}). Status: PASSED -> Strong foundational pillars for life stability." if len(k_planets) >= 2 else f"Kendra Houses contain {len(k_planets)} planets. Status: OK -> Balanced core life pillars."
        
        # 2. Trikona Check
        t_planets = house_occupants[5] + house_occupants[9]
        s2 = f"Trikona Houses (5,9) contain {len(t_planets)} planets: ({', '.join(t_planets) if t_planets else 'None'}). Status: PASSED -> Divine grace and high intellect active." if len(t_planets) >= 1 else f"Trikona Houses contain {len(t_planets)} planets. Status: OK -> Success driven by conscious self-efforts."
        ...
        # 4. Karako Bhava Nashaya Check (EXPLICIT REQ)
        ...
        # 5. Retrograde Check (EXCLUDING RAHU/KETU AS REQ)
        ...
        # 7. Step 7 Verification logic
        # Arudha Lagna (AL) = Sign of Lagna Lord + (Distance from Lagna to Lagna Lord)
...
def show_master_report():
    ...
    console.print(f"\n[bold yellow]🎯 MASTER 10-STEPS COMPREHENSIVE REPORT (VERIFIED LAHIRI AYANAMSHA)[/bold yellow]")
    print("===================================================================")
...
# ---------------- PDF GENERATION WITH TOTAL TELUGU & DEGREES ----------------
def create_pdf_report(days):
    ...
    # Gochara Forecast Scan with Accurate Degrees
    story.append(Paragraph("<b>📅 Daily Transits Scan (Gochara Hits Tracker):</b>", title_style))
    story.append(Spacer(1, 8))
        ...
        for tp, t in t_trop.items():
            for np, nlon in engine.n_ant.items():
                if abs(t["lon"] - nlon) < ORB:
                    day_text += f" - TROPICAL: {tp} ({t['lon']%30:.2f}°) hits Ant.{np} (<font color='green'>POSITIVE</font>)<br/>"
                    hit_found = True

        for tp, t in t_sid.items():
            for up, ulon in engine.n_up.items():
                if abs(t["lon"] - ulon) < ORB:
                    day_text += f" - SIDEREAL: {tp} ({t['lon']%30:.2f}°) hits {up} (<font color='red'>NEGATIVE</font>)<br/>"
                    hit_found = True
...
# Run Report & PDF at launch
show_master_report()
create_pdf_report(scan_days)

# ---------------- UI DASHBOARD WITH ACCURATE DEGREES ----------------
def get_chart():
    ...
    # Populate Natal planets with actual sign degrees
    ...
    # Populate Transit planets with actual sign degrees
