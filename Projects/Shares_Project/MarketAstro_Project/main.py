...
if __name__ == "__main__":
    ...
    with Live(screen=True, refresh_per_second=4) as live:
        while is_running:
            # 1. Get Data (Snapshot of all planets + Ascendant)
            data = astro.get_full_snapshot(view_date)
            
            # 2. Process Logic (Evaluating X, Y, and the 5 Trading Rules)
            logic = evaluate_market_rules(data, view_date)
            
            # 3. Get Dasa (Calculated using Moon's Longitude and Lord for balance)
            m_lon = data["Mo"]["lon"]
            m_lord = data["Mo"]["lord"]
