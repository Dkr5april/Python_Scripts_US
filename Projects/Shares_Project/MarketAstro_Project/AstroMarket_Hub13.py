...
# ==========================================================
# DYNAMIC ASTROLOGICAL WAVE TRENDS SIDEBAR GENERATOR
# ==========================================================
trend_nodes = analyze_market_trends(data)

if trend_nodes:
    st.sidebar.markdown("---")
    st.sidebar.subheader("📊 Astrological Wave Trends")
    
    for node in trend_nodes:
        st.sidebar.markdown(f"""
        <div style="border: 1px solid #282a36; padding: 10px; border-radius: 6px; background-color: #111217; margin-bottom: 4px;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="color:#ffffff; font-weight:bold; font-size:12px;">{node['label']}</span>
                <span style="color:{node['color']}; font-weight:bold; font-size:11px;">{node['trend']}</span>
            </div>
            <div style="margin-top:2px; margin-bottom:4px;">
                <small style="color:#6272a4;">{node['p_text']} in House {node['house']}</small>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        # Render the ultra-compact wave line vector model live
        fig = create_compact_trend_chart(node["graph_type"], node["color"])
        st.sidebar.plotly_chart(fig, use_container_width=True, key=f"side_trend_{node['label']}_{node['house']}_{node['dist']:.2f}")
...
# ==========================================================
# 8. DEEP CONTAINER PARSING FOR KP RULES
# ==========================================================
rules_rows = ""
...
# Scan backend configurations for nested dictionary formats safely
