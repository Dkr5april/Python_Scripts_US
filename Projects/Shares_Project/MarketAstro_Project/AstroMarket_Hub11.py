import streamlit as st
import streamlit.components.v1 as components
from datetime import datetime, timedelta

# Import your modules
import config
from engine.astro_engine import AstroEngine
from engine.market_dasa import get_market_dasa_levels
from logic.rules_processor import evaluate_market_rules

# 1. Page Configuration
st.set_page_config(page_title="Astro Market Dashboard", layout="wide")

# 2. Initialize Backend Engine
@st.cache_resource
def load_engine():
    return AstroEngine()

astro = load_engine()

# 3. Initialize Session State Memory (Locks coordinates to Mumbai on first load)
if "view_date" not in st.session_state:
    st.session_state.view_date = datetime(2026, 6, 14, 9, 30, 0)

if "lat" not in st.session_state or "lon" not in st.session_state:
    st.session_state.lat = 18.5800
    st.session_state.lon = 72.5000
    st.session_state.location_name = "Mumbai (Bombay)"

# 4. Sidebar Controls (Time, Location, & Screen Toggle Input)
st.sidebar.header("⏱️ Control Panel")

location_options = ["Mumbai (Bombay)", "Default (Machilipatnam)"]
try:
    current_idx = location_options.index(st.session_state.location_name)
except (ValueError, AttributeError):
    current_idx = 0

location_option = st.sidebar.selectbox(
    "Select Location",
    options=location_options,
    index=current_idx
)

# Shift coordinates only when the user manually alters the dropdown selection
if location_option != st.session_state.location_name:
    st.session_state.location_name = location_option
    if location_option == "Mumbai (Bombay)":
        st.session_state.lat = 18.5800
        st.session_state.lon = 72.5000
    else:
        st.session_state.lat = 16.1176
        st.session_state.lon = 80.9314
    st.rerun()

new_date = st.sidebar.date_input("Pick Date", value=st.session_state.view_date.date())
new_time = st.sidebar.time_input("Pick Time", value=st.session_state.view_date.time())
st.session_state.view_date = datetime.combine(new_date, new_time)

st.sidebar.subheader("Quick Navigation")
col_b1, col_b2 = st.sidebar.columns(2)
with col_b1:
    if st.button("⬅️ -5 Mins"):
        st.session_state.view_date -= timedelta(minutes=5)
        st.rerun()
    if st.button("⬅️ -1 Day"):
        st.session_state.view_date -= timedelta(days=1)
        st.rerun()
with col_b2:
    if st.button("+5 Mins ➡️"):
        st.session_state.view_date += timedelta(minutes=5)
        st.rerun()
    if st.button("+1 Day ➡️"):
        st.session_state.view_date += timedelta(days=1)
        st.rerun()

# SCREEN TOGGLE: Switch layout viewing modes smoothly
st.sidebar.markdown("---")
st.sidebar.subheader("🖥️ View View Mode")
view_mode = st.sidebar.radio(
    "Select Screen Content",
    options=["Standard Dashboard", "KP Rules Detailed Screen"],
    index=0
)

# 5. Enforce coordinates back into engine instance
astro.lat = st.session_state.lat
astro.lon = st.session_state.lon

if hasattr(config, 'LATITUDE'): config.LATITUDE = st.session_state.lat
if hasattr(config, 'LONGITUDE'): config.LONGITUDE = st.session_state.lon

view_date = st.session_state.view_date

# MATCHING MAIN.PY: Get raw engine snapshot and pass directly to rules processor
data = astro.get_full_snapshot(view_date)
logic = evaluate_market_rules(data, view_date)

m_lon = data["Mo"]["lon"]
m_lord = data["Mo"]["lord"]
dasa = get_market_dasa_levels(view_date, m_lon, m_lord)

# 6. Dynamic Color Highlighting for Shared Star Lords
palette = ["#ff5555", "#50fa7b", "#f1fa8c", "#bd93f9", "#ff79c6", "#8be9fd", "#ffb86c", "#ff6e6e", "#6272a4"]
unique_lords = list(set([details.get("lord") for details in data.values() if details.get("lord")]))
lord_color_map = {lord: palette[i % len(palette)] for i, lord in enumerate(unique_lords)}

lord_counts = {}
for details in data.values():
    l = details.get("lord")
    if l: lord_counts[l] = lord_counts.get(l, 0) + 1

# 7. Format Outputs & Build HTML Table
dasa_text = " > ".join(dasa) if isinstance(dasa, list) else str(dasa)

table_rows = ""
entity_order = ["Asc", "Su", "Mo", "Ma", "Me", "Ju", "Ve", "Sa", "Ra", "Ke", "Ur", "Ne", "Pl"]
available_keys = list(data.keys())

execution_order = []
for ent in entity_order:
    if ent in available_keys:
        execution_order.append(ent)
    elif ent == "Asc" and "Lagna" in available_keys:
        execution_order.append("Lagna")

for remaining in available_keys:
    if remaining not in execution_order and remaining not in ["Lagna", "Asc"]:
        execution_order.append(remaining)

for entity in execution_order:
    details = data[entity]
    
    if entity in ["Lagna", "Asc"]:
        ent_color = "#ff79c6"
    elif entity in ["Ur", "Ne", "Pl"]:
        ent_color = "#8be9fd"
    else:
        ent_color = "#ffffff"
        
    status_str = details.get("status", "DIR")
    status_color = "#ff5555" if status_str == "RETRO" else "#50fa7b"
    
    # Calculate display modulo on-the-fly to keep UI clean
    raw_lon = details.get('lon', 0.0)
    sign_relative_deg = raw_lon % 30
    house_num = details.get('house', '1' if entity in ["Lagna", "Asc"] else '-')
    
    star_name = details.get('star', 'N/A')
    lord_name = details.get('lord', 'N/A')
    
    star_display_color = "#ffffff"
    if lord_counts.get(lord_name, 0) > 1 and entity not in ["Lagna", "Asc"]:
        star_display_color = lord_color_map.get(lord_name, "#ffffff")
        
    table_rows += f"""
    <tr style="border-bottom: 1px solid #21262d;">
        <td style="padding:8px; color:{ent_color}; font-weight:bold;">{entity}</td>
        <td style="padding:8px; color:{star_display_color};">{star_name} ({lord_name})</td>
        <td style="padding:8px; color:#ffffff; font-weight:bold; text-align:center;">{house_num}</td>
        <td style="padding:8px; color:#ffffff; text-align:center;">{details.get('pada', '-')}</td>
        <td style="padding:8px; color:#ffffff;">{sign_relative_deg:.4f}°</td>
        <td style="padding:8px; color:{status_color}; font-weight:bold;">{status_str}</td>
    </tr>
    """

# ==========================================================
# 8. KP RULE EXTRACTOR 
# ==========================================================
rules_rows = ""
processed_keys = set()

def add_rule_line(label, value):
    global rules_rows
    text = str(value).strip()
    upper = text.upper()

    if "PASS" in upper:
        color = "#50fa7b"
        display = "PASS"
    elif "FAIL" in upper:
        color = "#ff5555"
        display = "FAIL"
    else:
        color = "#f1fa8c"
        display = text

    rules_rows += f"""
    <div style="margin-bottom:12px; font-size:15px; font-family:'Courier New';">
        <span style="color:white;">{label.strip()}:</span>
        <span style="color:{color}; font-weight:bold;">{display}</span>
    </div>
    """

# Scan backend dictionary
for key, value in logic.items():
    key_text = str(key)

    # Direct R1-R5 keys
    if key_text.upper().startswith(("R1", "R2", "R3", "R4", "R5")):
        add_rule_line(key_text, value)
        continue

    # RULE_RESULTS container matching nested string layouts
    if key_text.upper() in ["RULE_RESULTS", "RULES", "RESULTS"]:
        if isinstance(value, list):
            for item in value:
                txt = str(item)
                if txt.strip() and ":" in txt:
                    add_rule_line(txt.split(":")[0], txt.split(":")[-1])
        else:
            lines = str(value).split("\n")
            for line in lines:
                if ":" in line:
                    add_rule_line(line.split(":")[0], line.split(":")[-1])

# Backup calculation keys
x_chain_val = logic.get("X-Chain", logic.get("XC", "N/A"))
y_chain_val = logic.get("Y-Chain", logic.get("YC", "N/A"))

# ==========================================================
# 9. DYNAMIC SINGLE-SCREEN HTML GENERATION (BASED ON TOGGLE)
# ==========================================================

# Determine grid layouts and visibility switches dynamically using toggle selection
if view_mode == "KP Rules Detailed Screen":
    # Full Screen Layout for KP Trading Rules
    dashboard_grid_content = f"""
    <div style="width: 100%;">
        <div class="box-container" style="min-height: 520px; padding: 25px;">
            <div class="box-title" style="font-size: 16px; margin-bottom: 20px;">KP Trading Rules (Detailed Screen View)</div>
            <div style="margin-bottom: 24px; color:#ffb86c; font-size:18px; font-weight:bold; font-family:'Courier New'; border-bottom: 1px dashed #30363d; padding-bottom: 10px;">
                X: {logic.get('X', 'Mo')} | Y: {logic.get('Y', 'Mo')}
            </div>
            <div style="margin-top: 15px;">
                {rules_rows}
            </div>
            <div style="margin-top: 40px; border-top: 1px solid #38444d; padding-top: 20px;">
                <div style="color: #8b949e; font-size: 13px; text-transform: uppercase; margin-bottom: 10px; font-weight: bold;">Chain Depths Reference:</div>
                <span style="font-size:14px; margin-right: 30px; font-family:'Courier New';">X-Chain: <span style='color:#bd93f9; font-weight:bold;'>{x_chain_val}</span></span>
                <span style="font-size:14px; font-family:'Courier New';">Y-Chain: <span style='color:#bd93f9; font-weight:bold;'>{y_chain_val}</span></span>
            </div>
        </div>
    </div>
    """
else:
    # Default Dual Split-Screen Layout
    dashboard_grid_content = f"""
    <div class="dashboard-grid">
        
        <div class="box-container">
            <div class="box-title">Sidereal Celestial Metrics (Lahiri)</div>
            <table>
                <tr style="border-bottom: 2px solid #38444d; color:#ff79c6; font-weight:bold;">
                    <th style="padding:8px;">Entity</th>
                    <th style="padding:8px;">Star (Lord)</th>
                    <th style="padding:8px; text-align:center;">House</th>
                    <th style="padding:8px; text-align:center;">Pada</th>
                    <th style="padding:8px;">Deg</th>
                    <th style="padding:8px;">Status</th>
                </tr>
                {table_rows}
            </table>
        </div>

        <div>
            <div class="box-container">
                <div class="box-title">6-Hour Market Dasa</div>
                <div style="color:#8be9fd; font-weight:bold; margin-bottom:6px; font-size:13px;">Maha &gt; Antar &gt; Pratyantar:</div>
                <div style="color:#ff79c6; font-size:17px; font-weight:bold; letter-spacing:0.5px;">{dasa_text}</div>
            </div>

            <div class="box-container">
                <div class="box-title">KP Trading Rules</div>
                <div style="margin-bottom:14px; color:#ffb86c; font-size:15px; font-weight:bold;">X: {logic.get('X', 'Mo')} | Y: {logic.get('Y', 'Mo')}</div>
                {rules_rows}
            </div>

            <div class="box-container">
                <div class="box-title">Stellar Chain Depth</div>
                <div style="font-size:14px; margin-bottom:6px;">X-Chain: <span style='color:#bd93f9; font-weight:bold;'>{x_chain_val}</span></div>
                <div style="font-size:14px;">Y-Chain: <span style='color:#bd93f9; font-weight:bold;'>{y_chain_val}</span></div>
            </div>
        </div>

    </div>
    """

# Wrap content into complete canvas framework HTML block
complete_html_page = f"""
<!DOCTYPE html>
<html>
<head>
    <style>
        body {{
            background-color: #0c0f12;
            color: #ffffff;
            font-family: 'Courier New', Courier, monospace;
            margin: 0;
            padding: 10px;
        }}
        .header-banner {{
            background-color: #161b22;
            padding: 12px;
            border-radius: 4px;
            border: 1px solid #38444d;
            margin-bottom: 15px;
            font-size: 15px;
        }}
        .dashboard-grid {{
            display: grid;
            grid-template-columns: 1.25fr 0.75fr;
            gap: 15px;
        }}
        .box-container {{
            border: 1px solid #38444d;
            border-radius: 4px;
            padding: 15px;
            margin-bottom: 15px;
            background-color: #000000;
        }}
        .box-title {{
            color: #8b949e;
            border-bottom: 1px dashed #30363d;
            padding-bottom: 6px;
            margin-bottom: 12px;
            font-size: 14px;
            text-transform: uppercase;
            letter-spacing: 1px;
            font-weight: bold;
        }}
        table {{
            width: 100%;
            border-collapse: collapse;
            text-align: left;
        }}
    </style>
</head>
<body>

    <div class="header-banner">
        <span style="color:#ff79c6; font-weight:bold;">TARGET:</span> {view_date.strftime('%Y-%m-%d %H:%M:%S')} | 
        <span style="color:#f1fa8c; font-weight:bold;">LOCATION:</span> {st.session_state.location_name.upper()}
    </div>

    {dashboard_grid_content}

</body>
</html>
"""

# 10. Render Interface Canvas
components.html(complete_html_page, height=720, scrolling=False)