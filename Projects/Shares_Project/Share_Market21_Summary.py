...
def evaluate_market(data, view_date):
    ...
    if key_planets:
        for kp in key_planets:
            ...
            total_score += val
            max_possible += 10
            verdicts.append(f"[magenta]KEY: {kp} ({nature})[/]")
    ...
    # 3. PRIMARY FALLBACK
        ...
        for p_name in [x_lord, y_lord]:
            ...
            max_possible += 5
            verdicts.append(f"Primary {p_name}: H{h_dist}")
    ...
    elif abs_p > 10: strength = "MILD"
    else: strength = "NEUTRAL / VOLATILE"

    if percentage > 10:
        conclusion = f"{abs_p}% BULLISH ({strength})"
    elif percentage < -10:
        conclusion = f"{abs_p}% BEARISH ({strength})"
    else:
        conclusion = f"50% {strength}"

    verdicts.append("─" * 15)
    verdicts.append(f"[bold]{conclusion}[/]")
    return verdicts

# ---------------- POSTER GENERATOR ----------------
def generate_summary_poster(engine, loc_name, days=30):
    pdf = FPDF(orientation='L', unit='mm', format='A3')
    pdf.add_page()
    start_date = engine.view_date
    
    pdf.set_font("Arial", 'B', 26)
    pdf.cell(0, 20, f"30-DAY MARKET OUTLOOK POSTER - {start_date.strftime('%B %Y')}", ln=True, align='C')
    pdf.set_font("Arial", '', 14)
    pdf.cell(0, 10, f"Location: {loc_name} | Daily Snapshot at 09:30 AM", ln=True, align='C')
    pdf.ln(10)
    ...
    for i in range(days):
        ...
        res = evaluate_market(engine.get_data(), curr_dt)
        conclusion = res[-1].replace("[bold]", "").replace("[/]", "")

        # Color logic
        if "BULLISH" in conclusion: pdf.set_fill_color(210, 255, 210)
        elif "BEARISH" in conclusion: pdf.set_fill_color(255, 210, 210)
        else: pdf.set_fill_color(240, 240, 240)

        x, y = pdf.get_x(), pdf.get_y()
        pdf.rect(x, y, col_w - 3, row_h - 3, 'F')
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(col_w, 10, curr_dt.strftime('%d-%b (%a)'), ln=0, align='C')
        
        pdf.set_xy(x, y + 18)
        pdf.set_font("Arial", '', 10)
        pdf.multi_cell(col_w - 3, 8, conclusion, align='C')
        
        pdf.set_xy(x + col_w, y)
        if (i + 1) % 6 == 0: pdf.ln(row_h)

    fn = f"Market_Poster_{start_date.strftime('%Y%m%d')}.pdf"
    pdf.output(fn)
    engine.view_date = start_date
    console.print(f"[bold green]Poster Generated: {fn}[/]")
...
class SiderealEngine:
    ...
    def get_data(self):
        ...
        for pid, name in ALL_PLANETS:
            ...
            star_idx = int(res[0] / 13.33333333)
            p_data[name] = {"star": NAKSHATRAS[star_idx], "lord": LORDS[star_idx % 9], "lon": res[0], "rasi": RASI_NAMES[int(res[0] / 30)], "deg": res[0] % 30}
            if name == "Ra":
                k_lon = (res[0] + 180) % 360
                p_data["Ke"] = {"star": NAKSHATRAS[int(k_lon/13.33333333)], "lord": LORDS[int(k_lon/13.33333333)%9], "lon": k_lon, "rasi": RASI_NAMES[int(k_lon/30)], "deg": k_lon % 30}
        return p_data
...
mode = input("Select Mode: [1] Live View [2] 30-Day Summary Poster: ")

# Set time to 09:30 AM
if date_input:
    start_dt = datetime.strptime(date_input, "%Y-%m-%d").replace(hour=9, minute=30)
else:
    start_dt = datetime.now().replace(hour=9, minute=30, second=0, microsecond=0)

geolocator = Nominatim(user_agent="market_v20")
...
if mode == "2":
    generate_summary_poster(engine, loc_input)
else:
    running = True
    def make_layout():
        ...
        layout["main"].split_row(Layout(Panel(table), ratio=1), Layout(Panel("\n".join(evaluate_market(data, engine.view_date)), title="Logic"), ratio=1))
        return layout
