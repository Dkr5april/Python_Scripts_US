...
# Vimshottari Star Lords sequence
...
# Updated to include Outer Planets: Ur (7), Ne (8), Pl (9)
ALL_PLANETS = [
...
# ---------------- INPUTS ----------------
console.clear()
console.print("[bold magenta]=== FULL SIDEREAL TRACKER: 12-BODY SYSTEM ===[/bold magenta]\n")
...
class SiderealEngine:
    ...
    def get_data(self):
        ...
        for pid, name in ALL_PLANETS:
            ...
            # Rahu (Ra) and Ketu (Ke) always 180 deg apart
            if name == "Ra":
...
def make_layout():
    ...
    notif = (
        f"[bold cyan]TRANSIT WATCH[/]\n"
        f"Moon Star: {mo['star']} ({mo['lord']})\n"
        f"Next Pada: [bold green]{time_to_pada:.2f} hrs[/]\n"
        f"\n[dim white]Arrows: +/- Time[/]\n"
        f"[dim white]PgUp/PgDn: +/- Date[/]\n"
        f"[dim white]ESC/Q: Exit[/]"
    )
    ...
    layout["main"].split_row(
        ...
        Layout(Panel(notif, title="Navigation"), ratio=1)
    )
    return layout
