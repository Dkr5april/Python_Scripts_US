...
class MobileCopilot:
    def __init__(self):
        ...
        self.last_time = 0
        self.stitch_window = 5.0  # 5 seconds to bridge pauses
    ...
    def start_move(self, event): self._x = event.x; self._y = event.y
    def do_move(self, event):
        x = self.root.winfo_x() + (event.x - self._x)
        y = self.root.winfo_y() + (event.y - self._y)
        self.root.geometry(f"+{x}+{y}")

    def on_closing(self): self.is_running = False; self.root.destroy(); sys.exit(0)
    ...
    def get_ai_response(self, text):
        ...
        # --- STITCHING LOGIC (APPENDING ORDER) ---
        # If within the window, take the PREVIOUS text and ADD the new text to the end
        if (current_time - self.last_time) < self.stitch_window and self.last_transcript:
            full_query = f"{self.last_transcript} {new_text}".strip()
        else:
            full_query = new_text
        
        self.last_transcript = full_query
        self.last_time = current_time

        self.root.after(0, lambda: self.status.config(text="THINKING...", fg="yellow"))
        self.root.after(0, lambda: self.q_label.config(text=f"Q: {full_query}"))
        
        try:
            res = groq_client.chat.completions.create(
                model="llama-3.1-8b-instant",
                messages=[
                    {"role": "system", "content": f"SAP Expert. SUMMARY: 1 sentence. DETAILS: 3 technical bullets. Context: {RESUME_CONTEXT}"},
            ...
            if "DETAILS:" in raw:
                ...
                summary = raw; self.full_detail_cache = "No additional details."
