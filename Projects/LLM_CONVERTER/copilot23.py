...
class LevelUpCopilot:
    def __init__(self):
        ...
        self.root.geometry("700x700+300+50") # Increased height for debug console
        ...
        # Start Engines with Debug Tracking
        self.log("INITIALIZING ENGINES...")
        threading.Thread(target=self.audio_engine, name="AudioThread", daemon=True).start()
        threading.Thread(target=self.cloud_engine, name="CloudThread", daemon=True).start()
        threading.Thread(target=self.context_engine, name="ContextThread", daemon=True).start()
        
        self.root.mainloop()

    def log(self, msg):
        """Debug logger with millisecond precision"""
    ...
    def build_ui(self):
        ...
        tk.Label(self.root, text="TRACE LOGS", bg="#1a1a1a", fg="orange", font=("Arial", 7, "bold")).pack(fill="x")
        self.debug_console = tk.Text(self.root, bg="#111", fg="#666", font=("Consolas", 8), height=8)
    ...
    def toggle_mode(self):
        ...
        self.log(f"Switched to {'MIC' if self.mode == 1 else 'SYSTEM'} mode.")
    ...
    def audio_engine(self):
        ...
        def capture(device_id, target_mode, name):
            try:
                ...
                self.log(f"Audio Engine: Started {name} capture.")
                while self.is_running:
                    ...
                    if self.mode == target_mode:
                        ...
                        if vol > THRESHOLD:
                            processed = np_data[::2][::3] # Downsample
                            if not self.audio_queue.full(): 
                                # Tag with timestamp to track queue latency later
                                self.audio_queue.put((time.time(), processed.tobytes()))
            except Exception as e: self.log(f"Audio Error ({name}): {e}")
    ...
    def cloud_engine(self):
        while self.is_running:
            try:
                self.log("Cloud Engine: Connecting to Deepgram...")
                ...
                while self.is_running:
                    try:
                        born_at, chunk = self.audio_queue.get(timeout=1.0)
                        # Trace latency: Time between mic capture and sending to cloud
                        latency = time.time() - born_at
                        if latency > 0.5: self.log(f"TRACE: Queue Delay {latency:.2f}s")
    ...
    def context_engine(self):
        while self.is_running:
            time.sleep(0.1)
            # Reduced delay from 1.5 to 1.0 for snappier triggering
            if self.fragments and (time.time() - self.last_heard > 1.0):
                full_q = " ".join(self.fragments).strip()
                self.fragments = []
                self.log(f"Logic: Silence detected. Dispatching to AI...")
    ...
    def ask_ai(self, text):
        try:
            ...
            self.log(f"Groq: Response in {latency:.2f}s")
    ...
    def display_fresh(self, q, a, lat):
        ...
        self.text_display.insert(tk.END, f"[AI Latency: {lat:.2f}s]\n", "lat_style")
    ...
    def next_level(self):
        ...
        if lvl_str in self.full_response:
            self.log(f"UI: Switching to Level {lvl_str}")
            self.text_display.insert(tk.END, f"\n--- LEVEL {lvl_str} ---\n", "lvl_head")
            ...
            self.log(f"UI: Level {lvl_str} not generated by AI.")
