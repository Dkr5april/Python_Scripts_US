...
5. **FIXED**: Corrected the pandas usage: 'dayfirst=True' is only used in to_datetime, 
   and column overwriting is guaranteed.
"""
import sys
import random
import pandas as pd
...
def main():
    ...
    try:
        # *** FIX 1: Removed dayfirst=True from read_excel, as it's not supported here ***
    ...
    for index, row in df_input.iterrows():
        try:
            ...
            # Match Date Handling
            # *** FIX 2: Use dayfirst=True in to_datetime here to handle D/M/Y ***
            ...
            try:
                # Assuming time is consistently HH:MM:SS or HH:MM
                ...
                # Fallback or default if time parsing fails
                hm = time(6, 0)
    ...
    # ------------------ Output & File Write (Fixed Logic) ------------------
    if all_results:
        df_output = pd.DataFrame(all_results)
        df_input.reset_index(drop=True, inplace=True)
        df_output.reset_index(drop=True, inplace=True)

        # 1. Define the columns generated by the script
        GENERATED_COLS = list(df_output.columns)
        
        # 2. Drop the existing generated columns from the input DataFrame (Overwrite logic)
        ...
        # 3. Concatenate the cleaned input with the new output
        df_final = pd.concat([df_cleaned_input, df_output], axis=1)

        # === CRITICAL: Reformat date columns to DD/MM/YYYY string explicitly ===
        date_cols_to_reformat = ['Captain_A_DOB', 'Captain_B_DOB', 'Match_Date']
        for col in date_cols_to_reformat:
            if col in df_final.columns:
                try:
                    # *** FIX 3: Use errors='coerce' and dayfirst=True for correct D/M/Y handling ***
                    df_final[col] = pd.to_datetime(df_final[col], errors='coerce', dayfirst=True).dt.strftime('%d/%m/%Y')
                except Exception as format_e:
                    # Added a more specific warning for debugging, but let's re-run
                    print(f"Warning: Could not reformat column {col} to string. Error: {format_e}")
        # =================================================================================================
