...
and writes output to two separate sheets in the same file to preserve input formatting.

Key Fixes in this Version:
1. Prevents input date fields (DOB, Match Date) from being reformatted during the
   initial load by reading them as strings, ensuring the 'Input_Data_Original' sheet
   retains the exact source formatting.
2. The final date reformatting loop for the 'Analysis_Output' sheet has been removed
   as it is no longer required and caused unwanted modification of input columns.
"""
import sys
import random
import pandas as pd
...
try:
    ...
    print(f"{Colors.WARNING}ERROR: 'pyswisseph' module not found. Astrological calculations will use mock/default values.{Colors.ENDC}")
...
def birth_number(dob_str: str) -> int:
    ...
    try:
        # Note: This relies on the DD/MM/YYYY string created inside the main loop
...
def final_toss_prediction(toss_results):
    ...
    if a_wins >= 4:
        return f"{Colors.BOLD}{Colors.OKBLUE}Captain A ({a_name}) - STRONG MAJORITY ({a_wins}-{b_wins}){Colors.ENDC}"
    if b_wins >= 4:
        return f"{Colors.BOLD}{Colors.OKGREEN}Captain B ({b_name}) - STRONG MAJORITY ({b_wins}-{a_wins}){Colors.ENDC}"
    
    if a_wins > b_wins:
          return f"{Colors.OKBLUE}Captain A ({a_name}) - Simple Majority ({a_wins}-{b_wins}){Colors.ENDC}"
    if b_wins > a_wins:
          return f"{Colors.OKGREEN}Captain B ({b_name}) - Simple Majority ({b_wins}-{a_wins}){Colors.ENDC}"

    return f"{Colors.WARNING}Neutral - Tie/Mixed Results ({a_wins}-{b_wins}){Colors.ENDC}"
...
def predict_innings_trend(match_score: int, lp: int, nn: int, dp: int, pp: int, is_winner: bool) -> str:
    ...
    if start_score >= 14:
        start = f"{Colors.OKGREEN}Strong, aggressive start{Colors.ENDC}"
    elif start_score >= 8:
        start = f"{Colors.OKBLUE}Steady, watchful start{Colors.ENDC}"
    else:
        start = f"{Colors.WARNING}Slow, cautious start{Colors.ENDC}"
        
    # 2. Middle Overs (Name Number + Match Score)
    score_influence = 1 if match_score > 35 else 0 
    if nn + score_influence >= 8:
        middle = f"{Colors.OKGREEN}consistent, dominant middle overs{Colors.ENDC}"
    elif nn >= 4:
        middle = f"{Colors.OKBLUE}balanced flow with periodic power hitting{Colors.ENDC}"
    else:
        middle = f"{Colors.WARNING}slowdown with potential wicket clusters{Colors.ENDC}"

    # 3. Finish/End Result (Life Path + Win Probability)
    if is_winner:
        end = f"{Colors.BOLD}{Colors.OKGREEN}culminating in a DECISIVE finish{Colors.ENDC}"
    else:
        if lp >= 6:
            end = f"{Colors.OKBLUE}struggles but reaches a COMPETITIVE total{Colors.ENDC}"
        else:
            end = f"{Colors.FAIL}fizzle out towards a BELOW-PAR total{Colors.ENDC}"

    # Assemble the final narrative
    return f"{start} {Colors.BOLD}→{Colors.ENDC} {middle} {Colors.BOLD}→{Colors.ENDC} {end}"
...
def main():
    print(f"{Colors.HEADER}{Colors.BOLD}=== MASTER COMPLETE ASTRO–NUMEROLOGY ANALYSIS (IN-PLACE EXCEL I/O) ==={Colors.ENDC}\n")
    IO_FILE = "match_analysis_data.xlsx"
    
    # Define columns that must be read as strings to preserve user's formatting
    date_cols_to_keep_as_text = ['Captain_A_DOB', 'Captain_B_DOB', 'Match_Date']

    # ------------------ Initialization & Input ------------------
    try:
        # Load the file, explicitly telling Pandas to read date columns as strings
        df_input = pd.read_excel(IO_FILE, dtype={col: str for col in date_cols_to_keep_as_text})
        print(f"{Colors.OKBLUE}Successfully loaded {len(df_input)} matches from {Colors.BOLD}{IO_FILE}{Colors.ENDC}{Colors.OKBLUE}{Colors.ENDC}")
    ...
    for index, row in df_input.iterrows():
        try:
            ...
            # Since we loaded the dates as STRINGS, row.get() returns the raw text.
            # We now use pd.to_datetime here to convert the raw string into a datetime object for calculations.
            
            # Match Date Parsing
            match_date_raw = row.get('Match_Date', '01/01/2025')
            match_dt_obj = pd.to_datetime(match_date_raw, dayfirst=True)
            match_date = match_dt_obj.date() 
            match_date_str = match_dt_obj.strftime("%d/%m/%Y") # Formatted for numerology functions
            
            # Captain A DOB Parsing
            a_dob_raw = row.get('Captain_A_DOB', '01/01/1980')
            a_dob_dt = pd.to_datetime(a_dob_raw, dayfirst=True)
            a_dob = a_dob_dt.strftime("%d/%m/%Y") 

            # Captain B DOB Parsing
            b_dob_raw = row.get('Captain_B_DOB', '01/01/1980')
            b_dob_dt = pd.to_datetime(b_dob_raw, dayfirst=True)
            ...
            try:
                if ':' in match_time_str:
                    time_format = "%H:%M:%S" if match_time_str.count(':') == 2 else "%H:%M"
                    hm = datetime.strptime(match_time_str, time_format).time()
                else:
                    raise ValueError("Time format unexpected.")
            except ValueError:
                hm = time(6, 0)
            ...
            if score_a > score_b + 5:
                advantage = f"{Colors.BOLD}{Colors.OKBLUE}Captain A ({a_name}) - Strong Advantage{Colors.ENDC}"
                score_range = "180 - 200+"
            elif score_a > score_b:
                advantage = f"{Colors.OKBLUE}Captain A ({a_name}){Colors.ENDC}"
                score_range = "165 - 185"
            elif score_b > score_a + 5:
                advantage = f"{Colors.BOLD}{Colors.OKGREEN}Captain B ({b_name}) - Strong Advantage{Colors.ENDC}"
                score_range = "180 - 200+"
            elif score_b > score_a:
                advantage = f"{Colors.OKGREEN}Captain B ({b_name}){Colors.ENDC}"
                score_range = "165 - 185"
            else:
                advantage = f"{Colors.WARNING}Neutral / Balanced{Colors.ENDC}"
                score_range = "155 - 175"
            
            print(f"\n{Colors.BOLD}{Colors.HEADER}--- Processing Match {index + 1}: {team_a} vs {team_b} ({match_date_str}) ---{Colors.ENDC}")
            print(f"  {Colors.BOLD}Predicted Advantage:{Colors.ENDC} {advantage}")
            print(f"  {Colors.BOLD}Predicted Score Range:{Colors.ENDC} {score_range}")
            ...
            print(f"  {Colors.BOLD}Final Toss Prediction:{Colors.ENDC} {final_toss}")
            ...
            print(f"  {Colors.BOLD}Innings Trend A ({a_name}):{Colors.ENDC} {trend_a}")
            print(f"  {Colors.BOLD}Innings Trend B ({b_name}):{Colors.ENDC} {trend_b}")
            
            # --- 6. Collect Final Results for THIS ROW ---

            result = {
                'Overall_Advantage': advantage.split('-')[0].strip().replace(Colors.BOLD, '').replace(Colors.OKBLUE, '').replace(Colors.OKGREEN, '').replace(Colors.ENDC, '').replace(Colors.WARNING, ''),
                'Predicted_Score_Range': score_range,
                **toss_results, # Unpack the 6 toss methods
                'Final_Toss_Prediction': final_toss.split('-')[0].strip().replace(Colors.BOLD, '').replace(Colors.OKBLUE, '').replace(Colors.OKGREEN, '').replace(Colors.ENDC, '').replace(Colors.WARNING, ''),
                'Innings_Trend_A': trend_a.replace(Colors.BOLD, '').replace(Colors.OKBLUE, '').replace(Colors.OKGREEN, '').replace(Colors.ENDC, '').replace(Colors.WARNING, '').replace(Colors.FAIL, ''),
                'Innings_Trend_B': trend_b.replace(Colors.BOLD, '').replace(Colors.OKBLUE, '').replace(Colors.OKGREEN, '').replace(Colors.ENDC, '').replace(Colors.WARNING, '').replace(Colors.FAIL, ''),
            ...
            print(f"{Colors.FAIL}Error processing match at index {index}: {e}{Colors.ENDC}")
            continue
    ...
    if all_results:
        ...
        GENERATED_COLS = list(df_output.columns)
        cols_to_drop_from_input = [col for col in GENERATED_COLS if col in df_input.columns]
        
        # df_cleaned_input uses the raw string columns from df_input
        df_cleaned_input = df_input.drop(columns=cols_to_drop_from_input, errors='ignore')
        df_final = pd.concat([df_cleaned_input, df_output], axis=1)

        try:
            # Use ExcelWriter to write to multiple sheets
            with pd.ExcelWriter(IO_FILE, engine='xlsxwriter') as writer:
                # Write the original input data to the first sheet, preserving its raw string format
                df_cleaned_input.to_excel(writer, sheet_name='Input_Data_Original', index=False)
                
                # Write the full results (input + output) to a new sheet
                df_final.to_excel(writer, sheet_name='Analysis_Output', index=False)
            
            # These print statements must be inside the 'try' block to confirm success
            print(f"\n{Colors.OKGREEN}{Colors.BOLD}✅ Analysis complete! Results saved to **{IO_FILE}** in the 'Analysis_Output' sheet.{Colors.ENDC}")
            print(f"{Colors.HEADER}{Colors.BOLD}>> RUN prediction_report_generator.py NEXT FOR VISUAL CHARTS <<{Colors.ENDC}")

        except Exception as write_error:
            # This 'except' block handles errors during the file write process
            print(f"\n{Colors.FAIL}{Colors.BOLD}FATAL ERROR WRITING FILE: {write_error}{Colors.ENDC}")
            print(f"{Colors.FAIL}Please ensure the Excel file 'match_analysis_data.xlsx' is CLOSED before running the script. (Or check if 'xlsxwriter' is installed){Colors.ENDC}")
    
    # This 'else' block executes if 'if all_results:' was false
    else:
        print(f"\n{Colors.WARNING}❌ No results to write. Please check input file content.{Colors.ENDC}")
    
    # This final check is outside the 'if/else' structure
    if not swe:
        print(f"{Colors.WARNING}NOTE: Swiss Ephemeris was not found. Install 'pyswisseph' for full functionality.{Colors.ENDC}")
