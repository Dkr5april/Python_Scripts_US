...
Replaces Captain A/B with actual team names in predictions
"""
...
def colorize_prediction(text, team_a, team_b):
    """
    Replace Captain A/B with team names and color-code based on the original captain.
    """
    text = str(text)
    if "Captain A" in text:
        text = text.replace("Captain A", team_a)
        return f"{Colors.OKBLUE}{text}{Colors.ENDC}"
    elif "Captain B" in text:
        text = text.replace("Captain B", team_b)
        return f"{Colors.OKGREEN}{text}{Colors.ENDC}"
    else:
        # For neutral or other text
...
def generate_report():
    ...
    for idx in selected_indexes:
        ...
        # Replace Captain A/B with team names in predictions and color-code
        adv_text = colorize_prediction(row[adv_col], team_a, team_b)
        toss_text = colorize_prediction(row[toss_col], team_a, team_b)

        report_data.append({
            'Match ID': idx + 1,
            'Date': row[date_col],
            'Match': match_label,
            'Win Advantage': adv_text,
            'Toss Prediction': toss_text,
        })
    ...
    # Aggregated toss (using team names)
    ...
    for col in TOSS_METHOD_COLS:
        result = str(row.get(col, '')).strip().lower()
        if "captain a" in result:
            all_predictions.append(team_a)
        elif "captain b" in result:
